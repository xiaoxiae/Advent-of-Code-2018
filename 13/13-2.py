...
while True:
    ...
    while len(carts) != 0:
        ...
        # if the grid we are standing on is a cart, remove both
        if len(data[cart[1]][cart[0]]) > 1:
            # find which heap the cart is on
            cartHeap = carts if data[cart[1]][cart[0]] in carts else newCarts

            # delete the cart from the heap
            del(cartHeap[cartHeap.index(data[cart[1]][cart[0]])])

            # place whatever piece of grid it was holding back
            data[cart[1]][cart[0]] = data[cart[1]][cart[0]][4]

            # heapify the heap again
            heapq.heapify(cartHeap)
            continue
    ...
    if len(newCarts) == 1:
        print(str(newCarts[0][0])+","+str(newCarts[0][1]))
        quit()
